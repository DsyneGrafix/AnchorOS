...
CP-003 and CP-006 consume `SecurityCoreGateway`, a narrow public interface.
The production implementation is expected from the Security Core integration
layer. Calls include deterministic idempotency keys and fail closed when the
interface or its receipts are unavailable.
...
- The demonstration Security Core adapter is non-production; operational use
  requires the BOOT-0021 Security Core public implementation.
